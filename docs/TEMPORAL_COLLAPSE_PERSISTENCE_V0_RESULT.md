...
Adding post-confirmation states made it possible to distinguish confirmed collapse from persistent collapse regime.
...
This means raw COLLAPSE must appear across 2 consecutive raw frames before temporal COLLAPSE is emitted.

It also means temporal COLLAPSE must persist across 2 consecutive temporal frames before persistence is counted.
...
This means the system separates raw failure, confirmed collapse, and persistent regime.
...
The system now demonstrates both temporal confirmation and post-confirmation persistence.
...
The delay between raw collapse and temporal collapse can be interpreted as confirmation latency.

The first persistent collapse step can be interpreted as persistent divergence onset.
