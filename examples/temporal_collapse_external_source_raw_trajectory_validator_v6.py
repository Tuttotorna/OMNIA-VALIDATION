...
v3 read bounded raw ordered reference trajectories directly.
v4 applied the raw trajectory warning mechanism to external-style trajectories
   with source independence marked as not_verified.
v5 applied the raw trajectory warning mechanism to trajectories generated
   by a separate generator script.
v6 applies the same raw trajectory warning mechanism to declared
   external-source raw trajectory records.

This does not prove absolute external verification.
...
INPUT_PATH = DATA_DIR / "temporal_collapse_external_source_raw_trajectories_v6.jsonl"
OUTPUT_PATH = RESULTS_DIR / "temporal_collapse_external_source_raw_trajectory_validator_v6.json"
...
def main() -> None:
    ...
    payload = {
        "experiment": "temporal_collapse_external_source_raw_trajectory_validator_v6",
        "status": "v6_external_source_raw_trajectory_validation",
        "boundary": (
            "declared external-source raw ordered trajectory records; "
            "source_independence is external_source_declared, not external_source_verified"
        ),
        "claim": (
            "This script applies the Level 3 raw trajectory warning mechanism "
            "to declared external-source raw ordered trajectory records using "
            "visible signals, weights, thresholds, source labels, independence "
            "method labels, and transition evidence."
        ),
        ...
        "external_source_note": (
            "source_independence=external_source_declared means the source boundary "
            "is declared as external-source style. It does not mean external_source_verified "
            "or absolute independent verification."
        ),
        "weights": WEIGHTS,
    ...
    print("=" * 100)
    print("TEMPORAL COLLAPSE EXTERNAL-SOURCE RAW TRAJECTORY VALIDATOR — v6")
    print("=" * 100)
