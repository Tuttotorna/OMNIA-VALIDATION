...
external-style or independently generated raw ordered trajectory records.
...
v4 applies the same raw trajectory warning mechanism to external-style
raw ordered trajectories with explicit source labels.
...
INPUT_PATH = DATA_DIR / "temporal_collapse_external_raw_trajectories_v4.jsonl"
OUTPUT_PATH = RESULTS_DIR / "temporal_collapse_external_raw_trajectory_validator_v4.json"
...
def trajectory_source(events: list[dict]) -> str:
    sources = sorted({str(item.get("source", "unknown")) for item in events})
    ...
    return "MULTIPLE_SOURCES:" + ",".join(sources)


def trajectory_source_independence(events: list[dict]) -> str:
    values = sorted({
        str(item.get("source_independence", "not_declared"))
        for item in events
    })
...
def build_source_summary(results: list[dict]) -> list[dict]:
    ...
    for source, items in sorted(grouped.items()):
        ...
        if len(independence_values) == 1:
            independence_status = independence_values[0]
        else:
            independence_status = "mixed:" + ",".join(independence_values)
...
def main() -> None:
    ...
    payload = {
        "experiment": "temporal_collapse_external_raw_trajectory_validator_v4",
        "status": "v4_external_raw_trajectory_validation",
        "boundary": (
            "external-style raw ordered trajectory records; "
            "source independence explicitly tracked"
        ),
        "claim": (
            "This script applies the Level 3 v3 raw trajectory warning "
            "mechanism to external-style raw ordered trajectory records using "
            "visible signals, weights, thresholds, source labels, and transition evidence."
        ),
        ...
        "source_independence_note": (
            ...
            "If source_independence is not_verified, the result must be read as "
            "external-style validation, not verified independent validation."
        ),
        "weights": WEIGHTS,
    ...
    print("=" * 100)
    print("TEMPORAL COLLAPSE EXTERNAL RAW TRAJECTORY VALIDATOR — v4")
    print("=" * 100)
