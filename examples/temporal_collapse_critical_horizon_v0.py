...
RESULTS_PATH = os.path.join(
    RESULTS_DIR,
    "temporal_collapse_critical_horizon_v0.json",
)
...
def evaluate_persistent_trajectory(
    ...
    horizon_error = (
        first_persistent_collapse - expected_horizon
        if first_persistent_collapse is not None
        and expected_horizon is not None
        else None
    )
...
def estimate_critical_horizon(
    ...
    expected_horizons = [
        ...
        for item in observations
        if item["expected_horizon"] is not None
    ]

    expected_horizon = (
        expected_horizons[0]
        if expected_horizons
        else None
    )
...
def main():
    ...
    horizon_errors = [
        item["mean_horizon_error"]
        for item in observable_results
        if item["mean_horizon_error"] is not None
    ]

    exact_horizon_match_count = sum(
        1
        for item in observable_results
        if item["mean_horizon_error"] == 0
    )
    ...
    summary = {
        ...
        "exact_horizon_match_count": exact_horizon_match_count,
        "exact_horizon_match_rate": safe_div(
            exact_horizon_match_count,
            len(observable_results),
        ),
        "mean_horizon_error": (
            mean(horizon_errors)
    ...
    status = (
        ...
        if (
            ...
            and summary["exact_horizon_match_rate"] >= 0.90
    ...
    payload = {
        ...
        "reproduction_command": (
            "python examples/temporal_collapse_critical_horizon_v0.py"
        ),
    }
    ...
    print("=" * 80)
    print("OMNIA-VALIDATION - Temporal Collapse Critical Horizon v0")
    print("=" * 80)
    print()
    ...
    for item in horizon_results:
        print(
            ...
            f"expected={item['expected_horizon']} "
            f"mean_error={item['mean_horizon_error']}"
        )
    ...
    if status == "PASS":
        print(
            "PASS - critical temporal horizon matched the estimated "
            "observability rule."
        )
    else:
        print(
            "CHECK - critical temporal horizon did not fully match "
            "the estimated observability rule."
        )
