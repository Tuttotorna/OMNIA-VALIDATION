...
# =============================================================================
# OMNIA-VALIDATION — Observer Family Geometry v3
# =============================================================================
...
RESULTS_PATH = RESULTS_DIR / "observer_family_geometry_v3.json"
...
def observer_response(observer_name):
    ...
    for sample in SAMPLES:
        vectors.append(weighted_common_vector(sample, weights))
    ...
    for feature in feature_names:
        ...
        aggregate[mean_key] = mean_value
        aggregate[var_key] = variance
...
def observer_similarity(observer_a, observer_b):
    ...
    # Anti-saturation smoothing:
    # keeps true zeros possible, but compresses extreme distances slightly.
    # This helps expose medium relations without flattening all observers.
...
def classify_geometry(pairwise, uniqueness):
    ...
    summary["observer_relation_entropy"] = observer_entropy(summary)
...
def pass_condition(summary):
    return (
        summary["observer_count"] >= 12
        and summary["family_count"] >= 5
        and summary["redundant_pair_count"] >= 6
        and summary["medium_pair_count"] >= 12
        and summary["diverse_pair_count"] >= 20
        and summary["same_family_medium_or_redundant_count"] >= 8
        and summary["cross_family_diverse_count"] >= 15
        and summary["geometry_continuity_index"] >= 0.10
        and summary["observer_relation_entropy"] >= 0.50
        and 0.35 < summary["average_distance"] < 0.75
    )
...
def main():
    ...
    output = {
        "experiment_name": "observer_family_geometry_v3",
        "version": VERSION,
        "date_utc": datetime.now(timezone.utc).isoformat(),
        "domain": "observer_family_geometry",
        "purpose": (
            "Measure observer-space continuity using soft shared latent "
            "feature channels, interpolation observers, and anti-saturation "
            "normalization."
        ),
        ...
        "status": status,
        "pass_condition": (
            "observer_count >= 12 and family_count >= 5 and "
            "redundant_pair_count >= 6 and medium_pair_count >= 12 and "
            "diverse_pair_count >= 20 and "
            "same_family_medium_or_redundant_count >= 8 and "
            "cross_family_diverse_count >= 15 and "
            "geometry_continuity_index >= 0.10 and "
            "observer_relation_entropy >= 0.50 and "
            "0.35 < average_distance < 0.75"
        ),
        "main_insight": (
            "Observer-space geometry should contain redundant, medium, "
            "and diverse relations rather than collapsing into binary "
            "near/far separation."
        ),
        "limitations": [
    ...
    print("=" * 80)
    print("OMNIA-VALIDATION — Observer Family Geometry v3")
    print("=" * 80)
    ...
    print(f"  Geometry continuity index:              {summary['geometry_continuity_index']}")
    print(f"  Observer relation entropy:              {summary['observer_relation_entropy']}")
    print()
