...
RESULTS_PATH = os.path.join(
    RESULTS_DIR,
    "temporal_collapse_dynamic_horizon_v0.json",
)
...
def predicted_horizon_from_raw_run(raw_run):
    return (
        raw_run["start"]
        + CONFIRMATION_WINDOW
        + PERSISTENCE_WINDOW
...
def evaluate_trajectory(name, states):
    ...
    predicted_horizons = [
        predicted_horizon_from_raw_run(run)
        for run in valid_raw_runs
    ]

    first_observed_persistent = first_persistent_index(
        temporal_actions,
        PERSISTENCE_WINDOW,
    )

    first_predicted_horizon = (
        predicted_horizons[0]
        if predicted_horizons
        else None
    )

    horizon_error = (
        first_observed_persistent - first_predicted_horizon
        if first_observed_persistent is not None
        and first_predicted_horizon is not None
        else None
    )
    ...
    horizon_fragmentation = len(valid_raw_runs)
    ...
    if name in {
        ...
        if first_observed_persistent is None:
            dynamic_status = "CHECK"
    ...
    return {
        ...
        "first_predicted_horizon": first_predicted_horizon,
        "first_observed_persistent": first_observed_persistent,
        "horizon_error": horizon_error,
        "raw_collapse_runs": raw_collapse_runs,
        "valid_raw_runs": valid_raw_runs,
        "temporal_collapse_runs": temporal_collapse_runs,
...
def main():
    ...
    observable_results = [
        item
        for item in trajectory_results
        if item["first_observed_persistent"] is not None
    ]

    horizon_errors = [
        item["horizon_error"]
        for item in trajectory_results
        if item["horizon_error"] is not None
    ]
    ...
    summary = {
        ...
        "max_abs_horizon_error": (
            max(abs(value) for value in horizon_errors)
            if horizon_errors
            else None
        ),
        ...
        "corrected_static_rule": (
            "critical_horizon = first_raw_collapse + "
            "confirmation_window + persistence_window - 2"
        ),
    }
    ...
    status = (
        ...
        if (
            ...
            and summary["mean_horizon_error"] == 0
            and summary["max_abs_horizon_error"] == 0
    ...
    print("=" * 80)
    print("OMNIA-VALIDATION - Temporal Collapse Dynamic Horizon v0")
    print("=" * 80)
    print()
    ...
    for item in trajectory_results:
        print(
            ...
            f"predicted={item['first_predicted_horizon']} "
            f"observed={item['first_observed_persistent']} "
            f"error={item['horizon_error']} "
    ...
    if status == "PASS":
        print(
            "PASS - dynamic horizon remained predictable under "
            "non-stationary temporal structure."
        )
    else:
        print(
            "CHECK - dynamic horizon exposed temporal instability "
            "under non-stationary structure."
        )
