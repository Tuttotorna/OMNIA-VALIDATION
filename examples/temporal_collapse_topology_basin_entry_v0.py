...
def find_basin_entries_from_sequence(sequence, max_depth=2):
    source_class = classify(sequence)["classification"]
...
def main():
    ...
    status = (
        ...
        if (
            fragmented_dominance
            and len(fully_persistent_entries) >= 3
            and "FRAGMENTED_LOCAL_COLLAPSE"
            in stability["target_example_totals"]
        )
        else "CHECK"
    )
    ...
    payload = {
        ...
        "interpretation": (
            "This experiment measures whether "
            "FRAGMENTED_LOCAL_COLLAPSE behaves as the canonical "
            "entry state into the confirmed-collapse basin."
        ),
        "reproduction_command": (
            "python examples/"
            "temporal_collapse_topology_basin_entry_v0.py"
        ),
    }
    ...
    if status == "PASS":
        print(
            "PASS - FRAGMENTED_LOCAL_COLLAPSE behaved as "
            "the dominant basin-entry state into confirmed-collapse "
            "topology."
        )
    else:
        print(
            "CHECK - basin-entry dominance was not fully stable "
            "under the tested variants."
        )
