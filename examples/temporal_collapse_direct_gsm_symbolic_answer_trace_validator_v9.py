...
direct GSM-Symbolic answer traces mapped into raw ordered structural
trajectory events.
...
This does not evaluate semantic truth.

Correctness is tracked as evidence, not as the final measurement.
...
INPUT_PATH = DATA_DIR / "temporal_collapse_direct_gsm_symbolic_answer_traces_v9.jsonl"
OUTPUT_PATH = RESULTS_DIR / "temporal_collapse_direct_gsm_symbolic_answer_trace_validator_v9.json"
...
def compute_collapse_similarity(events: list[dict], boundary_proximity: float) -> float:
    ...
    broken_marker = 1.0 if any(
        ...
        for item in events
        for marker in {"BROKEN", "FAIL", "NULL", "COLLAPSE"}
    ) else 0.0
...
def build_source_summary(results: list[dict]) -> list[dict]:
    ...
    for (source, source_record_type, mapping_method, model_name), items in sorted(grouped.items()):
...
def main() -> None:
    ...
    model_names = sorted({
        item.get("model_name", "not_declared")
        for item in results
    })

    payload = {
        "experiment": "temporal_collapse_direct_gsm_symbolic_answer_trace_validator_v9",
        "status": "v9_direct_answer_trace_mapping",
        "boundary": (
            "direct GSM-Symbolic answer traces mapped into raw ordered "
            "structural trajectory events"
        ),
        "claim": (
            "This script applies the Level 3 raw trajectory warning mechanism "
            "to GSM-Symbolic answer traces mapped into raw ordered structural "
            "trajectory events. It does not claim that OMNIA solves GSM-Symbolic, "
            "does not infer semantic truth, and does not replace benchmark correctness."
        ),
        ...
        "external_source_note": (
            "source_independence=external_source_verified means the answer traces "
            "are mapped from a public/documentable benchmark source. "
            "independence_method=direct_answer_trace_mapping means the records "
            "include direct answer-trace fields such as model_name, run_id, "
            "response_id, expected_answer, model_final_answer, and is_correct. "
            "Correctness is reported as evidence through correctness_profile; "
    ...
    print("=" * 100)
    print("TEMPORAL COLLAPSE DIRECT GSM-SYMBOLIC ANSWER TRACE VALIDATOR — v9")
    print("=" * 100)
